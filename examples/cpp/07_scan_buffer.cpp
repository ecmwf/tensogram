...
static std::vector<std::uint8_t> make_message(const char* param, int step) {
    ...
    std::snprintf(json, sizeof(json),
        R"({"version":3,"descriptors":[{"type":"ntensor","ndim":1,"shape":[10],"strides":[1],"dtype":"float32","byte_order":"little","encoding":"none","filter":"none","compression":"none"}],"base":[{"mars":{"param":"%s","step":%d}}]})",
        param, step);

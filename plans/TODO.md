...
## Multi-Language Support
  ...
  - [ ] **fortran-interface — shallow `iso_c_binding` binding over the C FFI**
    ...
    - **Fortran 2008 library** (PLAN_FORTRAN.md §10.8): the rank-generic
      surface is explicit per-rank specifics generated by the C
      preprocessor (`src/tgm_*.inc`), not assumed-rank + `select rank`, so
      the library compiles clean under `-std=f2008 -Wall -Wextra -Werror`
      (CI gate `make fortran-f2008-check`). The RAII handle types are kept,
      so gfortran's `f08/0011` advisory means example/app code builds under
      `-std=f2018`; the library is the conformance unit.

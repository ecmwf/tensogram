...
- Role-based classifier instead of direction-only.  A backward preamble validation has the SAME 24-byte range as a forward preamble (both at `[msg_start, msg_start + 24)`), so the classifier cannot distinguish them by range alone.  The four-valued `ScanRole` tags via observation context: `fwd_preamble` for ranges at known message_starts, `bwd_postamble` for ranges at message_ends, and `bwd_preamble_validation` is determined by the `RoundBuilder` state machine (a `fwd_preamble`-shaped fetch following a `bwd_postamble` in the same round).  The state machine handles all four arrival shapes the bidirectional walker emits: `[fwd, bwd_pa, bwd_pre_val]`, `[fwd, bwd_pa]` (validation format error), `[fwd]` (backward already disabled), `[bwd_pa, fwd]` (paired-fetch reorder).

...
| `tensogram/file.rs` — open methods | `open_source`, `open_remote`, `open_source_async`, `open_remote_async` gain `scan_opts: Option<&RemoteScanOptions>` (default `None` collapses to forward-only).  Local-file backends accept the value and silently ignore it.  The redundant `RemoteBackend::open` / `open_async` wrappers (which only forwarded to the `_with_scan_opts` siblings with `Default`) are dropped. |
...
- `Option<&RemoteScanOptions>` over a builder.  The TODO entry called for this shape; in practice `None` collapses to `Default::default()` in one line and the API remains a single function instead of fanning out into `open_remote_builder().bidirectional(true).build()`.

...
fn default_metadata() -> GlobalMetadata {
    GlobalMetadata {        ..Default::default()
    }
}
...
fn decode_preserves_metadata_version() {
    let meta = GlobalMetadata {        ..Default::default()
    };
    let desc = make_descriptor(vec![1], Dtype::Uint8);
...
fn decode_range_single_range_uncompressed() {
    ...
    let result = tensogram_wasm::decode_range(&msg, 0, &ranges).unwrap();
...
fn decode_range_odd_length_rejected() {
    ...
    odd.set_index(1, 2);
    odd.set_index(2, 4);
    assert!(tensogram_wasm::decode_range(&msg, 0, &odd).is_err());
}
...
fn decode_range_filter_shuffle_rejected() {
    ...
    let res = tensogram_wasm::decode_range(&msg, 0, &ranges);
...
fn streaming_encoder_single_object_round_trip() {
    let meta_js = serde_wasm_bindgen::to_value(&default_metadata()).unwrap();
    let mut enc = tensogram_wasm::StreamingEncoder::new(
        meta_js,
        Some(true),
        None,
        None,
...
fn streaming_encoder_finish_closes_handle() {
    ...
    let mut enc2 = tensogram_wasm::StreamingEncoder::new(
        meta_js2, None, None, None, None, None, None, None, None,
    )
    .unwrap();
...
fn streaming_encoder_consecutive_preceders_rejected() {
    let meta_js = serde_wasm_bindgen::to_value(&default_metadata()).unwrap();
    let mut enc = tensogram_wasm::StreamingEncoder::new(
        meta_js, None, None, None, None, None, None, None, None,
    )
    .unwrap();
...
fn streaming_encoder_callback_invoked_during_construction() {
    ...
    let (cb, chunks) = make_collector();
    let meta_js = serde_wasm_bindgen::to_value(&default_metadata()).unwrap();
    let _enc = tensogram_wasm::StreamingEncoder::new(
        meta_js,
        Some(false),
        Some(cb),
        None,
...
fn streaming_encoder_callback_error_propagates() {
    ...
    let meta_js = serde_wasm_bindgen::to_value(&default_metadata()).unwrap();
    let result = tensogram_wasm::StreamingEncoder::new(
        meta_js,
        Some(false),
        Some(bad_cb),
        None,
...
fn streaming_encoder_rejects_non_function_callback() {
    ...
    let enc = tensogram_wasm::StreamingEncoder::new(
        meta_js,
        None,
        Some(cb),
        None,

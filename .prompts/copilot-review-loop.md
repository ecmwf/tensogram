...
3. **Read every comment in the new review.** For each thread, either fix or reject:
   ...
   - **Reject** if the finding is a false positive, a misreading of the code, a misinterpretation of a deliberate decision, or otherwise does not warrant a change. Do NOT silently apply suggestions that would degrade the code, contradict documented design, or revert deliberate work. A rejected finding gets a reply that names the reason in one sentence.

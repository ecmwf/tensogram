...
Three new harnesses share a fixture roster and a
`cold_open_plus_operation_plus_close` sample contract so the
NDJSON sidecars stack up cell-for-cell:

- Rust — `cargo run -p tensogram-benchmarks --bin remote-scan-metrics`
  (deterministic single-pass) plus `cargo bench -p tensogram-benchmarks
  --bench remote_scan` (Vitest-style multi-sample wall-clock for headline
  cells).
- Python — `python rust/benchmarks/python/bench_remote_scan.py`
  (`--quick`, `--headline`, `--mode {sync,async}`, `--json`).
- TypeScript — `npm --prefix typescript run bench:remote-metrics`
  (full matrix) and `bench:remote-wallclock` (Vitest `bench()` for
  headline cells).
...
`examples/typescript/18_remote_scan_trace.ts` demonstrate subscribing
to the dispatcher's `tensogram::remote_scan` events
(`mode`, `hop`, `gap-closed`, `footer-eager`, ...).

### Decided — Remote bidirectional walker stays opt-in

Benchmarks across all three languages showed the bidirectional walker
strictly fetches more bytes than forward-only on every measured cell.
Defaults remain `bidirectional=false` across Rust, Python, and
TypeScript.  The full verdict, per-cell numbers, and the structural
failure modes (`object_store` range coalescing on Rust + Python;
discarded eager footer fetches on TypeScript) are recorded in
...
opt-out under any future default flip:
...
- `docs/src/guide/remote-access.md` and
  ...
  default verdict and the reader-side migration story (no wire-format
  bump; opt out with `bidirectional=False`).
- `docs/src/guide/benchmark-results-remote-scan.md` new page
  reproducing run metadata + headline numbers.

...
`fpm.toml` / `CMakeLists.txt` — then greps the tree for stragglers, separating
dependency *constraint ranges* (which encode a deliberate `<` ceiling and need
human review) from unexpected leftovers. Exposed as `make bump-version
VERSION=X.Y.Z`. The companion `make version-check` is a no-edit CI guard that
fails when any manifest drifts from the `VERSION` file. The generated,
git-ignored `typescript/wasm/package.json` is deliberately left untouched (it
is regenerated from the crate version at build time). The version
...
real-network workloads the open-time scan and tail / middle access
are roughly half the wall-clock of forward-only.

Existing `.tgm` files gain the speedup automatically — the walker is
reader-side only, with no migration, no re-encoding, and no wire-format
bump.  Force forward-only with `RemoteScanOptions { bidirectional: false }`
(Rust), `bidirectional=False` (Python), or
`{ bidirectional: false }` (TypeScript) when an adversarial server
might serve disagreeing forward/backward reads or when the consumer
specifically wants serial Range fetches.

The Rust + Python remote backend uses two independent `get_range`
futures joined via `tokio::join!` instead of `object_store::get_ranges`
to avoid the coalescer's 1 MiB merge gap collapsing paired probes
into one large slab fetch.  TypeScript gates the eager footer-region
fetch on the just-validated preamble's flags so header-indexed
messages don't pay a discarded-fetch round trip.

A new full-discovery scanner `scan_pipelined_async` runs entirely
off the state mutex, accumulates layouts in local `Vec`s, and
commits via `record_forward_hop` / `record_backward_hop` only after
the loop terminates (cursors meet, target index reached, or a
walker anomaly bails).  Returning `Ok(false)` on snapshot drift
ensures concurrent callers' partial state is never exposed as a
final result; the new
`bidir_message_count_race_with_partial_target` regression test
pins this invariant.

### Added — Remote-scan walker microbench across Rust, Python, and TypeScript

Three benchmark harnesses (Rust Criterion + metrics bin, Python
custom argparse, TypeScript Vitest + Node runner) share a fixture
roster and a `cold_open_plus_operation_plus_close` sample contract.
NDJSON sidecars at `target/remote-scan-bench/{rust,python,typescript}.ndjson`
share an identical schema for cross-language analysis.

New fixtures (`thousand-msg.tgm`, `hundred-msg-footer.tgm`,
`thousand-msg-footer.tgm`, `streaming-tail.tgm`) generated by
`tests/remote-parity/tools/gen_fixtures.py`.

### Added — Remote-scan tracing examples

`examples/rust/src/bin/18_remote_scan_trace.rs` and
`examples/typescript/18_remote_scan_trace.ts` subscribe to the
dispatcher's `tensogram::remote_scan` events (`mode`, `hop`,
`gap-closed`, `footer-eager`, ...).

### Fixed — Bidirectional opt-out chain across three languages

Three latent bugs would have silently overridden `bidirectional=False`
opt-out under any default change:

- `python/bindings/src/lib.rs:scan_opts_for` collapsed `false` to
  `None`, which the Rust layer reroutes through
  `RemoteScanOptions::default()`.  Fixed to always emit explicit
...
- `typescript/src/file.ts` validated `concurrency: 1` against the
  literal `bidirectional === true`, letting `{concurrency: 1}` bypass
...
message backward.  Validator now requires
...
- `plans/WIRE_FORMAT.md §9.3` rewritten with the format-vs-transport
  taxonomy: in-memory `ScanOptions` vs remote `RemoteScanOptions`.
- `docs/src/guide/remote-access.md` and
  `docs/src/guide/typescript-api.md` updated with the bidirectional
  default and the reader-side migration story.

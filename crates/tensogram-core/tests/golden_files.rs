...
//!
//! These files are generated by `generate_golden_files()` below and checked
//! into the repository under `tests/golden/`. Any language binding can decode
//! the same files to verify interoperability.
...
/// Ensure golden files exist. Thread-safe, runs at most once per test binary.
fn ensure_golden_files() {
    GENERATE.call_once(|| {
...
/// Generate all golden files. Idempotent — always produces identical bytes.
fn generate_golden_files() {
    ...
    // 1. Simple message: single float32 tensor [4], no compression
    {
        ...
        std::fs::write(dir.join("simple_f32.tgm"), &msg).unwrap();
    }
    ...
    // 2. Multi-object: 3 tensors with different dtypes
    {
        ...
        .unwrap();
        std::fs::write(dir.join("multi_object.tgm"), &msg).unwrap();
    }
    ...
    // 3. Message with metadata (MARS namespace)
    {
        ...
        let msg = encode::encode(&meta, &[(&desc, &payload)], &EncodeOptions::default()).unwrap();
        std::fs::write(dir.join("mars_metadata.tgm"), &msg).unwrap();
    }
    ...
    // 4. Multi-message file
    {
        ...
        std::fs::write(dir.join("multi_message.tgm"), &multi).unwrap();
    }
    ...
    // 5. Hash verification message (xxh3)
    {
        ...
        std::fs::write(dir.join("hash_xxh3.tgm"), &msg).unwrap();
    }
}

#[test]
fn test_generate_golden_files() {
    // Regenerate to ensure they exist
    generate_golden_files();

    // Verify they are deterministic (generate twice, compare)
    let dir = golden_dir();
    let first = std::fs::read(dir.join("simple_f32.tgm")).unwrap();
    generate_golden_files();
    let second = std::fs::read(dir.join("simple_f32.tgm")).unwrap();
    assert_eq!(
        first, second,
        "golden files must be byte-for-byte deterministic"
    );
}
